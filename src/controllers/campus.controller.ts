...
export class CampusController {
  ...
  registerNewCampus = async (req: Request, res: Response) => {
    ...
    try {
      const newCampusData: number = await this.campusRepository.registerCampus(address, city);
      res.json({messager: "Novo campus IFS registrado com sucesso!", id: newCampusData}).status(201);
    } catch (err) {
      console.log(err);
    }
  ...
  getAllCampus = async (req: Request, res: Response) => {
    const campusList: CampusDTO[] | null = await this.campusRepository.getCampusList();
